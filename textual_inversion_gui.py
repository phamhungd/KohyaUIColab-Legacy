...
def train_model(
    ...
    run_cmd = [f'python', '/content/gdrive/MyDrive/SD-Data/KohyaGUI/train_textual_inversion.py']
    if v2:
        run_cmd.append('--v2')
    ...
    run_cmd.append(f'--token_string={token_string}')
    run_cmd.append(f'--init_word={init_word}')
    run_cmd.append(f'--num_vectors_per_token={num_vectors_per_token}')
    if not weights == '':
        run_cmd.append(f'--weights={weights}')
    if template == 'object template':
        run_cmd.append(f'--use_object_template')
    elif template == 'style template':
        run_cmd.append(f'--use_style_template')
