...
def caption_images(
    ...
    run_cmd = ['python', '/content/gdrive/MyDrive/SD-Data/KohyaGUI/finetune/make_captions.py']
    if not model_id == '':
        run_cmd.append(f'--model_id={model_id}')
    run_cmd.append(f'--batch_size={int(batch_size)}')
    run_cmd.append(f'--max_data_loader_n_workers={int(max_data_loader_n_workers)}')
    run_cmd.append(f'--max_length={int(max_length)}')
    if caption_ext != '':
        run_cmd.append(f'--caption_extension={caption_ext}')
    run_cmd.append(str(train_data_dir))
